...
export function FileTree({ projectId, onNodeSelect }: FileTreeProps) {
  ...
  // Auto expand ancestors of active node (so refresh can stay on the same file)
  useEffect(() => {
    ...
    setExpandedIds((prev) => {
      ...
      while (currentId) {
        ...
        if (!current) break;
        if (current.parent_id && !next.has(current.parent_id)) {
  ...
  // System roots (depth=0): 正文 / 笔记
  const rootKindById = useMemo(() => {
    const byId = new Map(nodes.map((n) => [n.id, n] as const));
    ...
    const getKind = (id: string): string | undefined => {
      ...
      if (!n.parent_id) {
        ...
        cache.set(id, meta.root_kind);
        return meta.root_kind;
      }
      const kind = getKind(n.parent_id);
      cache.set(id, kind);
      return kind;
    };

    nodes.forEach((n) => {
      cache.set(n.id, getKind(n.id));
    });

    return cache;
  }, [nodes]);

  const systemRootIds = useMemo(() => {
    const ids = new Set<string>();
    nodes.forEach((n) => {
      const meta = (n.metadata || {}) as { system_root?: boolean };
      if (n.type === "FOLDER" && !n.parent_id && meta.system_root) ids.add(n.id);
    });
    return ids;
  }, [nodes]);

  const manuscriptRootId = useMemo(() => {
    for (const n of nodes) {
      const meta = (n.metadata || {}) as { system_root?: boolean; root_kind?: string };
      if (n.type === "FOLDER" && !n.parent_id && meta.system_root && meta.root_kind === "MANUSCRIPT") {
        return n.id;
      }
    }
    return null;
  ...
  const handleRename = useCallback(
    (nodeId: string, newTitle: string) => {
      if (systemRootIds.has(nodeId)) {
        toast.info("一级目录由系统固定，不能重命名");
        setEditingId(null);
        return;
      }
      if (newTitle.trim()) {
  ...
  const handleDelete = useCallback(
    ...
    [deleteNode, activeNodeId, setActiveNode, systemRootIds]
  );

  // Handle create node
  const handleCreate = useCallback(
    (type: NodeType, parentId: string | null = null) => {
      // Prevent creating at root when system roots exist; default to current folder or 正文
      let resolvedParentId = parentId;
      if (!resolvedParentId && manuscriptRootId) {
        const active = nodes.find((n) => n.id === activeNodeId) || null;
        resolvedParentId =
          active?.type === "FOLDER" ? active.id : active?.parent_id || manuscriptRootId;
      }

      const rootKind = resolvedParentId
        ? rootKindById.get(resolvedParentId)
        : undefined;
      ...
      const siblings = getSiblings(nodes, resolvedParentId);
      ...
      if (resolvedParentId) {
        setExpandedIds((prev) => new Set(prev).add(resolvedParentId));
      }
    },
    [createNode, projectId, nodes, activeNodeId, manuscriptRootId, rootKindById]
  );
  ...
  const handleDragEnd = useCallback(
    (event: DragEndEvent) => {
      ...
      if (systemRootIds.has(activeNode.id)) return;

      // Determine new parent and order
      let newParentId: string | null;
      let newOrder: string;

      const overIsSystemRoot = systemRootIds.has(overNode.id);

      if (overNode.type === "FOLDER" && (expandedIds.has(overNode.id) || overIsSystemRoot)) {
        // Drop into folder (system root can accept drops even if collapsed) - make it the first child
      ...
      // Prevent creating new root-level nodes when system roots exist
      if (manuscriptRootId && !newParentId) {
        return;
      }
    ...
    [flatNodes, expandedIds, nodes, moveNode, systemRootIds, manuscriptRootId]
  );
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === "F2" && activeNodeId) {
        e.preventDefault();
        if (systemRootIds.has(activeNodeId)) {
          toast.info("一级目录由系统固定，不能重命名");
          return;
        }
  ...
  }, [activeNodeId, editingId, nodes, handleCreate, handleDelete, systemRootIds]);
            ...
            <div className="p-2">
              {visibleNodes.length === 0 ? (
                ...
                visibleNodes.map((node) => {
                  const isSystemRoot = systemRootIds.has(node.id);
                  const rootKind = rootKindById.get(node.id);
                  const createFileLabel = rootKind === "NOTES" ? "新建文档" : "新建场景";
                  const createFolderLabel = rootKind === "NOTES" ? "新建文件夹" : "新建章节";
                  ...
                  return (
                    <TreeNodeItem
                      ...
                      disableDrag={isSystemRoot}
                      disableRename={isSystemRoot}
                      disableDelete={isSystemRoot}
                      createFileLabel={createFileLabel}
                      createFolderLabel={createFolderLabel}
                      onSelect={() => handleSelect(node)}
                      onToggleExpand={() => toggleExpand(node.id)}
                      onStartEdit={() => !isSystemRoot && setEditingId(node.id)}
                      onRename={(title) => handleRename(node.id, title)}
                      onDelete={() => handleDelete(node.id)}
                      onCreate={(type) =>
                        handleCreate(type, node.type === "FOLDER" ? node.id : node.parent_id)
                      }
                    />
                  );
                })
