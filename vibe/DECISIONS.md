...
### D011: API 响应格式统一
- **日期**: 2026-01-03
- **问题**: API 路由中存在大量重复的认证检查和响应格式代码
- **决策**: 创建 `src/lib/api/` 模块，包含统一的响应工具函数和认证中间件
- **原因**: 
  - 减少重复代码，提高可维护性
  - 统一错误码和响应格式
  - 便于后续添加新的 API 路由
- **影响**: 所有 API 路由使用统一的 `successResponse`、`errorResponse` 等函数

### D012: OAuth 组件抽取
- **日期**: 2026-01-03
- **问题**: Login 和 Signup 页面有大量重复的 OAuth 按钮和逻辑
- **决策**: 创建 `OAuthButtons` 组件和 `GoogleIcon` 图标组件
- **原因**:
  - 减少代码重复
  - 便于统一修改 OAuth 提供商
  - 保持 UI 一致性
- **影响**: 认证页面代码更简洁，OAuth 逻辑集中管理

### D013: 日期格式化工具
- **日期**: 2026-01-03
- **问题**: 日期格式化逻辑分散，且需要处理无效日期
- **决策**: 创建 `formatRelativeTime` 工具函数
- **原因**:
  - 统一日期显示格式
  - 安全处理 null 和无效日期
  - 默认使用中文 locale
