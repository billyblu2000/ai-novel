# AI Novel Studio - 开发进度

> 最后更新: 2026-01-03

## 整体进度

| 阶段 | 状态 | 完成度 |
|------|------|--------|
| 项目初始化 | ✅ 已完成 | 100% |
| 数据库 & 认证 | ✅ 已完成 | 100% |
| 核心 UI 框架 | ✅ 已完成 | 100% |
| 文件树 (Binder) | ✅ 已完成 | 100% |
| 编辑器 | ✅ 已完成 | 100% |
| 实体管理 | ✅ 已完成 | 100% |
| 时间线 | 🔲 未开始 | 0% |
| 优化 & 收尾 | 🔲 未开始 | 0% |

> **AI 功能**：详见 [PROGRESS_AI.md](./PROGRESS_AI.md)

---

## Phase 1: 项目初始化

### 1.1 环境搭建
- [x] 创建 Next.js 15 项目 (App Router, TypeScript)
- [x] 配置 Tailwind CSS
- [x] 安装并配置 Shadcn/ui
- [x] 配置 ESLint + Prettier
- [x] 创建基础目录结构 (`src/app`, `src/components`, `src/lib`, `src/types`)
- [x] 首次部署到 Vercel (尽早验证部署流程)

### 1.2 基础配置
- [x] 配置 Zustand store 基础结构
- [x] 配置 TanStack Query provider
- [x] 配置 Sonner (Toast)
- [x] 配置字体 (Inter, Noto Serif SC, JetBrains Mono)
- [x] 配置 Dark Mode (next-themes)

### 1.3 Supabase 初始化
- [x] 创建 Supabase 项目 (需用户在 Supabase Dashboard 创建)
- [x] 配置环境变量 (`.env.local`)
- [x] 安装 Supabase JS Client
- [x] 配置 Drizzle ORM

---

## Phase 2: 数据库 & 认证

### 2.1 数据库 Schema
- [x] 创建 `profiles` 表 + RLS
- [x] 创建 `projects` 表 + RLS
- [x] 创建 `nodes` 表 + RLS
- [x] 创建 `entities` 表 + RLS
- [x] 创建 `mentions` 表 + RLS
- [x] 创建 `node_versions` 表 + RLS
- [x] 配置级联删除约束
- [x] 创建 Drizzle schema 文件

### 2.2 认证系统
- [x] 配置 Supabase Auth
- [x] 实现登录页面 (`/login`)
- [x] 实现注册页面 (`/signup`)
- [x] 实现 OAuth (GitHub, Google)
- [x] 实现 Auth Middleware (保护路由)
- [x] 实现登出功能
- [x] 创建 Profile 自动创建 trigger

---

## Phase 3: 核心 UI 框架

### 3.1 布局组件
- [x] 创建 `AppShell` 三栏布局组件
- [x] 创建 `LeftSidebar` 组件 (可折叠)
- [x] 创建 `RightSidebar` 组件 (可折叠)
- [x] 创建 `MainContent` 组件
- [x] 实现响应式断点逻辑
- [x] 实现侧边栏折叠动画

### 3.2 Landing Page
- [x] 创建首页 (`/`)
- [x] Hero 区域
- [x] 功能介绍
- [x] CTA 按钮

### 3.3 项目列表页
- [x] 创建项目列表页 (`/projects`)
- [x] 项目卡片组件
- [x] 创建项目 Dialog
- [x] 删除项目确认 Dialog
- [x] 空状态展示

---

## Phase 4: 文件树 (Binder)

### 4.1 数据层
- [x] 实现 `useNodes` hook (TanStack Query)
- [x] 实现节点 CRUD API routes
- [x] 实现乐观更新逻辑
- [x] 实现 fractional indexing 排序

### 4.2 UI 组件
- [x] 创建 `FileTree` 组件
- [x] 创建 `TreeNode` 组件 (FOLDER/FILE 图标)
- [x] 实现 @dnd-kit 拖拽排序
- [x] 实现跨层级拖拽
- [x] 实现右键菜单 (新建/重命名/删除)
- [x] 实现快捷键 (Ctrl+N, F2, Delete)
- [x] 实现节点展开/折叠

### 4.3 项目仪表盘
- [x] 创建仪表盘页面 (`/editor/[id]`)
- [x] 统计卡片 (字数/章节/场景/实体)
- [x] 最近编辑列表
- [x] 写作进度展示

---

## Phase 5: 编辑器

### 5.1 Tiptap 基础
- [x] 安装 Tiptap 及扩展
- [x] 创建 `Editor` 组件
- [x] 配置基础扩展 (Document, Paragraph, Text, History)
- [x] 配置 Markdown 快捷键
- [x] 实现内容自动保存 (防抖 3s)
- [x] 实现 `Ctrl+S` 手动保存

### 5.2 FOLDER 视图
- [x] 创建大纲编辑器 (编辑 `outline` 字段)
- [x] 创建子节点卡片列表
- [x] 实现卡片点击跳转

### 5.3 FILE 视图
- [x] 创建正文编辑器
- [x] 实现字数统计
- [x] 实现场景元数据编辑 (timestamp, location, status)

### 5.4 版本历史
- [ ] 实现版本创建逻辑 (5分钟间隔 + 200字符变化)
- [ ] 创建版本历史面板
- [ ] 实现版本预览
- [ ] 实现版本恢复

---

## Phase 6: 实体管理

### 6.1 数据层
- [x] 实现 `useEntities` hook
- [x] 实现实体 CRUD API routes
- [x] 实现 Mentions 更新逻辑

### 6.2 Right Sidebar
- [x] 创建 `EntitySidebar` 组件
- [x] 实现 Context Mode (当前场景关联实体)
- [x] 实现 Browse Mode (按类型分组)
- [x] 创建实体卡片组件
- [x] 实现实体详情展开

### 6.3 实体 CRUD
- [x] 创建新实体 Dialog
- [x] 实现实体编辑 (内联)
- [x] 实现实体删除
- [x] 实现别名管理
- [x] 实现属性 (attributes) 编辑

### 6.4 实体高亮
- [x] 实现 Aho-Corasick 匹配 (Web Worker)
- [x] 创建 Tiptap 高亮扩展
- [x] 实现 Hover 卡片 (Tooltip)
- [x] 实现右键"忽略此匹配"
- [x] 存储 `ignored_entities` 到 metadata

---

## Phase 7: 时间线

### 8.1 数据层
- [ ] 实现时间线查询 (带 timestamp 的 FILE 节点)

### 8.2 UI 组件
- [ ] 创建 `Timeline` 组件
- [ ] 创建时间线节点卡片
- [ ] 实现点击跳转到场景
- [ ] 实现空状态提示

---

## Phase 8: 优化 & 收尾

### 9.1 离线支持
- [ ] 配置 idb-keyval
- [ ] 实现离线队列
- [ ] 实现网络恢复后同步
- [ ] 实现多设备冲突检测

### 9.2 性能优化
- [ ] 编辑器懒加载
- [ ] 文件树虚拟滚动 (如节点过多)
- [ ] 图片懒加载

### 9.3 最终测试
- [ ] 全流程手动测试
- [ ] 修复发现的 Bug

---

## 变更日志

| 日期 | 变更内容 |
|------|----------|
| 2026-01-03 | 完成 Phase 6.1-6.4 实体管理功能 |
| 2026-01-03 | 完成 Phase 5.1-5.3 编辑器功能 |
| 2026-01-03 | 完成 Phase 4.3 项目仪表盘 |
| 2026-01-03 | 完成 Phase 4.2 文件树 UI 组件 |
| 2026-01-03 | 完成 Phase 4.1 节点数据层 |
| 2026-01-03 | 代码重构：抽取公共组件和工具函数 |
| 2026-01-03 | 完成 Phase 3.3 项目列表页 |
| 2026-01-03 | 完成 Phase 3.2 Landing Page |
| 2026-01-03 | 完成 Phase 3.1 布局组件 |
| 2026-01-03 | 完成 Phase 2.2 认证系统 |
| 2026-01-03 | 完成 Phase 2.1 数据库 Schema |
| 2026-01-03 | 完成 Phase 1.3 Supabase 初始化 |
| 2026-01-03 | 完成 Phase 1.2 基础配置 |
| 2026-01-03 | 完成 Phase 1.1 环境搭建 |
| 2026-01-03 | 初始化进度文档 |
